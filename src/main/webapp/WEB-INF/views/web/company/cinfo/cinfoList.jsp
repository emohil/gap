								...
								<table width="100%" border="0" cellspacing="0" cellpadding="0" class="dataList">
									...
									<c:forEach items="${datas}" var="data" varStatus="status">
										<tr>
											...
											<td><a class=linkStyle href="<%=basePath%>company/${companyno}/cinfo/detail.html?id=${data.id}" target="_blank">${data.title}</a></td>
											<td><a class=linkStyle href="<%=basePath%>company/${companyno}/cinfo/detail.html?id=${data.id}" target="_blank">${data.keyword}</a></td>
										</tr>
									</c:forEach>
								</table>
							</td>
