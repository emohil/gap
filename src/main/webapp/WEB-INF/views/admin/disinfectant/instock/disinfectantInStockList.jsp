			...
			<tr>
				<td>
					<table border="0" cellspacing="0" cellpadding="0" class="subNav mCenter" style="width:100%;">
			      		<tr>
			        		<td class="font12Blue p10"><span class="font12Blue_B">您当前所在位置：消毒剂入库记录</td>
			      		</tr>
			   		 </table>
				</td>
			</tr>
			...
			<tr>
				<td>
					<table cellpadding="0" cellspacing="0" style="width:100%;">
						...
						<tr>
         					 <td class="dataListTitle"><span class="pl10">消毒剂入库记录</span></td>
       					</tr>
						<tr>
								...
								<table width="100%" border="0" cellspacing="0" cellpadding="0" class="dataList">
						            ...
						            <c:forEach items="${datas}" var="data" varStatus="status">
						              	<tr>
							                ...
							                <td>${data.disp.inuserId}</td>
							                <td>${data.disp.indate}</td>
							                <td>${data.disp.status}</td>
							                <td>
							                	<c:if test="${data.status == 0 }">
							                		<a class=linkStyle href="<%=basePath%>admin/disinfectant/instock/approve.html?id=${data.id}" onclick="return confirm('确定批准吗?')">批准</a>
													&nbsp;|&nbsp;
							                		<a class=linkStyle href="<%=basePath%>admin/disinfectant/instock/edit.html?id=${data.id}">修改</a>
													&nbsp;|&nbsp;
							                		<a class=linkStyle href="<%=basePath%>admin/disinfectant/instock/delete.html?id=${data.id}" onclick="return confirm('确定删除吗?')">删除</a>
							                	</c:if>
							                	<c:if test="${data.status == 2 }">
							                		<a class=linkStyle href="<%=basePath%>admin/disinfectant/instock/disp.html?id=${data.id}">查看</a>
							                		&nbsp;|&nbsp;
							                		<a class=linkStyle href="<%=basePath%>admin/disinfectant/instock/nullify.html?id=${data.id}" onclick="return confirm('确定作废吗?')">作废</a>
							                	</c:if>
