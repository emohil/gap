...
<head>
    ...
    <script type="text/javascript">
    	JGAP.on(window, 'load', function() {
	    	validator.regist({id : "stockId", name : "库存兽药"}, "notnull");
	    	validator.regist({id : "registerId", name : "兽药用途"}, "notnull");
	   	...
	   	function subform2(flag) {
        	...
        	if (typeof(form._onsubmit) == "function" && !form._onsubmit() 
       		...
       		$.ajax({
			   type: "POST",
			   url: "<%=basePath%>admin/medicine/outstock/checkOutStock.html",
        ...
        function doStockList() {
			var record = window.showModalDialog("<%=basePath%>admin/medicine/stock/select/list.html", '', 'dialogWidth:650px; dialogHeight:400px; edge:Raised; center:yes; help:no; status:no;scroll=no;');
			if (!record) return;
			...
			<tr>
				<td>
					<table border="0" cellspacing="0" cellpadding="0" class="subNav mCenter" style="width:100%;">
			      		<tr>
			        		<td class="font12Blue p10"><span class="font12Blue_B">您当前所在位置：兽药出库</td>
			      		</tr>
			   		 </table>
				</td>
			</tr>
			<tr>
				<td>
					<table style="width:100%;">
						<tr>
							<td class="dataListTitle"><span class="pl10">兽药出库</span></td>
						</tr>
						<tr>
							<td>
								<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="dataList">
									<tr>
								    	<td width="15%" class=forumrow><div align="right">兽药名称：</div></td>
